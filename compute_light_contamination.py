...
# --------------------------------------------------
# light-contamination:
#   contam = N(valid_pair & (fi==light OR fj==light)) / N(valid_pair)
#
# Weighted:
#   denom = sum_w(valid_pair)
#   num   = sum_w(valid_pair & (fi==light OR fj==light))
#   err   = sqrt(sum(w_pass^2)) / denom
# --------------------------------------------------
def weighted_pair_light_contamination_oriented(pair_indices, truth_flavs, weights, light_code=0):
    ...
    for ev_p, ev_f, w in zip(pair_indices, truth_flavs, weights):
        ...
        # numerator: at least one is light
        if (fi == light_code) or (fj == light_code):
            w_num  += w
            w2_num += w * w
...
def print_block(title, rows):
    print(title)
    print("njets, light_contam, error")
    for nj, p, e in rows:
...
# --------------------------------------------------
# Run light-contamination per method, per region
# --------------------------------------------------
LIGHT_CODE = 0  # 0 = light (u, d, s, gluon)
...
for method in ["chi2", "mdrs", "misms"]:
    ...
    for nj in range(2, 11):
        ...
        lcont, e, wnum, wden, n_valid, ex_pairs, ex_flavs = weighted_pair_light_contamination_oriented(
            pair_idx[method][m],
        ...
        rows.append((nj, lcont, e))

    print_block(
        f"{method}: light-contamination = N(valid_pair & (fi=={LIGHT_CODE} OR fj=={LIGHT_CODE})) / N(valid_pair)",
        rows
    )
