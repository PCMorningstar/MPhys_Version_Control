...
| input_histfilelist_path       | string | Path to list of histogram files for reading ````listOfSystematic``` if stored separately from tree. If not set the input filelist path is used |
...
| skip_empty_files | bool | If set to true, FastFrames will ignore ROOT files without the reco_tree_name TTree. It can be overriden per sample. Note that if set to ```true``` every single input file will be opened and checked which can lead to some performance issues, especially on remote files. Default is ```false```.|
...
| profile     | list of dicts     | List of TProfile histograms between 2 reco-level variables to produce. The dict must have 2 keys: ```x``` and ```Y``` for variables on x and y (the one for the mean calculation) axes. ```numbering_sequence``` block is supported (see details below) |
...
| merge_underflow_overflow | str | Defines what to do with underflow and overflow bins, whether they should be merged to the first/last visible bins or not. The available options are: ```none```, ```underflow```, ```overflow```, ```both```. The default option is ```None```. The options are not case sensitive. Override the value from ```General``` block.|
...
| skip_empty_files | bool | If set to true, FastFrames will ignore ROOT files without reco_tree_name TTree. It overrides the default from general block. |
