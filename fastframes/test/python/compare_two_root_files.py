...
"""
from ROOT import TFile, TTree, TH1D, TH2D, TH3D, TDirectory, TProfile
import sys
...
def compare_tprofiles(profile1 : TProfile, profile2 : TProfile) -> str:
    """
    Compare two TProfile histograms. If they are different, return a string explaining the first difference found. If identical, return None
    """
    if profile1.GetNbinsX() != profile2.GetNbinsX():
        return "Profiles have different number of bins in x"
    for i in range(profile1.GetNbinsX()+2):
        if not floats_are_equal(profile1.GetBinContent(i), profile2.GetBinContent(i)):
            return f"Profiles have different bin content at bin {i}: {profile1.GetBinContent(i)} != {profile2.GetBinContent(i)}"
    return None
...
def compare_histograms_in_folder(tfile1 : TFile, tfile2 : TFile, folder : str, histo_type : str = "TH1D"):
    ...
    histograms1 = get_list_of_objects_in_tdirectory(tfile1.Get(folder) if folder else tfile1, histo_type)
    histograms2 = get_list_of_objects_in_tdirectory(tfile2.Get(folder) if folder else tfile2, histo_type)
    ...
    for hist_name in histograms1:
        address = f"{folder}/{hist_name}" if folder else hist_name
        hist1 = tfile1.Get(address)
        hist2 = tfile2.Get(address)
...
def compare_all_histograms_in_files(file1 : TFile, file2 : TFile, folders_to_ignore : list[str] = []) -> str:
    ...
    for folder in folders1:
        ...
        comparison_3D = compare_histograms_in_folder(file1, file2, folder, "TProfile")
        if comparison_3D:
            return comparison_3D
