 ...
 */
 ...
 void MainFrame::executeHistograms() {
     ...
     for (const auto& isample : m_config->samples()) {
         ...
         if (isample->hasTruth() || m_config->splitProcessingPerUniqueSample()) {
             ...
             this->writeHistosToFile(finalSystHistos, RegionHisto("truth"), finalCutflowContainers, isample, true);
         }
 ...
 std::tuple<std::vector<SystematicHisto>,
     ...
     RegionHisto finalTruthHistos("truth");
     ...
     for (const auto& iUniqueSampleID : sample->uniqueSampleIDs()) {
         ...
         if (!truthHistos.variableHistos().empty()) {
             if (finalTruthHistos.variableHistos().empty()) {
                 LOG(INFO) << "Triggering the event loop for the truth tree\n";
                 for (const auto& ivariable : truthHistos.variableHistos()) {
                     finalTruthHistos.variableHistos().emplace_back(ivariable.name());
                     finalTruthHistos.variableHistos().back().copyHisto(ivariable.histo());
                 }
                 for (const auto& ivariable2D : truthHistos.variableHistos2D()) {
                     finalTruthHistos.variableHistos2D().emplace_back(ivariable2D.name());
                     finalTruthHistos.variableHistos2D().back().copyHisto(ivariable2D.histo());
                 }
                 for (const auto& ivariableProfile : truthHistos.variableHistosProfile()) {
                     finalTruthHistos.variableHistosProfile().emplace_back(ivariableProfile.name());
                     finalTruthHistos.variableHistosProfile().back().copyHisto(ivariableProfile.histo());
                 }
                 for (const auto& ivariable3D : truthHistos.variableHistos3D()) {
                     finalTruthHistos.variableHistos3D().emplace_back(ivariable3D.name());
                     finalTruthHistos.variableHistos3D().back().copyHisto(ivariable3D.histo());
                 }
             } else {
                 LOG(INFO) << "Merging truth, triggering the event loop for the truth trees!\n";
                 if (finalTruthHistos.variableHistos().size() != truthHistos.variableHistos().size()) {
                     LOG(ERROR) << "Sizes of truth histograms do not match!\n";
                     throw std::runtime_error("");
                 }
                 for (std::size_t ihist = 0; ihist < truthHistos.variableHistos().size(); ++ihist) {
                     finalTruthHistos.variableHistos().at(ihist).mergeHisto(truthHistos.variableHistos().at(ihist).histo());
                 }
                 for (std::size_t ihist = 0; ihist < truthHistos.variableHistos2D().size(); ++ihist) {
                     finalTruthHistos.variableHistos2D().at(ihist).mergeHisto(truthHistos.variableHistos2D().at(ihist).histo());
                 }
                 for (std::size_t ihist = 0; ihist < truthHistos.variableHistosProfile().size(); ++ihist) {
                     finalTruthHistos.variableHistosProfile().at(ihist).mergeHisto(truthHistos.variableHistosProfile().at(ihist).histo());
                 }
                 for (std::size_t ihist = 0; ihist < truthHistos.variableHistos3D().size(); ++ihist) {
                     finalTruthHistos.variableHistos3D().at(ihist).mergeHisto(truthHistos.variableHistos3D().at(ihist).histo());
                 }
             }
         }
         ++uniqueSampleN;
 ...
 std::tuple<std::vector<SystematicHisto>,
     ...
     if (selectedFilePaths.empty()) {
         ...
         return std::make_tuple(std::vector<SystematicHisto>{}, RegionHisto("truth"), std::vector<CutflowContainer>{}, tmp, nullptr, std::move(truthChains));
     }
     ...
     this->addTruthBranchesToMap(selectedFilePaths.at(0), sample);
 
     RegionHisto truthHistos("truth");
 ...
 std::tuple<std::vector<SystematicHisto>,
     ...
     // we could use any file from the list, use the first one
     // no need to add the truth branches sunce this cannot be processed with all unique samples
 ...
 void MainFrame::writeHistosToFile(const std::vector<SystematicHisto>& histos,
     ...
     // 1D histos
     for (const auto& itruthHist : truthHistos.variableHistos()) {
     ...
     // 2D histos
     for (const auto& itruthHist2D : truthHistos.variableHistos2D()) {
         const std::string truthHistoName2D = StringOperations::replaceString(itruthHist2D.name(), "_NOSYS", "");
         const auto vars = Utils::get2DVariablesByNameTruth(sample, itruthHist2D.name());
         std::unique_ptr<TH2D> histo(static_cast<TH2D*>(itruthHist2D.histoUniquePtr()->Clone()));
         Utils::mergeUnderOverFlow2D(histo.get(), vars.first.underOverFlowType(), vars.second.underOverFlowType());
         histo->GetXaxis()->SetTitle(vars.first.name().c_str());
         histo->GetYaxis()->SetTitle(vars.second.name().c_str());
         out->cd();
         histo->Write(truthHistoName2D.c_str());
     }
 
     // TProfile histos
     for (const auto& itruthHistProfile : truthHistos.variableHistosProfile()) {
         const std::string truthHistoNameProfile = StringOperations::replaceString(itruthHistProfile.name(), "_NOSYS", "");
         out->cd();
         itruthHistProfile.histoUniquePtr()->Write(truthHistoNameProfile.c_str());
     }
 
     // 3D histos
     for (const auto& itruthHist3D : truthHistos.variableHistos3D()) {
         const std::string truthHistoName3D = StringOperations::replaceString(itruthHist3D.name(), "_NOSYS", "");
         out->cd();
         itruthHist3D.histoUniquePtr()->Write(truthHistoName3D.c_str());
     }
 ...
 std::vector<std::pair<std::unique_ptr<TChain>, std::unique_ptr<TTreeIndex> > > MainFrame::connectTruthTrees(std::unique_ptr<TChain>& chain,
     ...
     for (const auto& itruth : sample->uniqueTruthTreeNamesForMatching()) {
         ...
         LOG(INFO) << "Done building reco truth index\n";
 ...
 RegionHisto MainFrame::processTruthHistos(const std::vector<std::string>& filePaths,
                                           const std::shared_ptr<Sample>& sample,
                                           const UniqueSampleID& id) {
 
     RegionHisto result("truth");
     ...
     for (const auto& itruth : sample->truths()) {
         ...
         for (const auto& ivariable : itruth->variables()) {
             ...
             result.addVariableHisto(std::move(hist));
         }
 
         // add 2D histograms
         for (const auto& ivar2D : itruth->variableCombinations()) {
             const Variable& var1 = itruth->variableByName(ivar2D.first);
             const Variable& var2 = itruth->variableByName(ivar2D.second);
 
             const std::string name = itruth->name() + "_" + var1.name() + "_vs_" + var2.name();
             VariableHisto2D variableHisto2D(name);
             ROOT::RDF::RResultPtr<TH2D> histogram2D = this->book2DhistoTruth(mainNode, var1, var2, itruth);
 
             variableHisto2D.setHisto(histogram2D);
 
             result.addVariableHisto2D(std::move(variableHisto2D));
         }
 
         // add TProfile histograms
         for (const auto& iprofile : itruth->variablesForProfile()) {
             const Variable& var1 = itruth->variableByName(iprofile.first);
             const Variable& var2 = itruth->variableByName(iprofile.second);
 
             const std::string name = itruth->name() + "_profile_" + var1.name() + "_vs_" + var2.name();
             VariableHistoProfile variableHistoProfile(name);
             ROOT::RDF::RResultPtr<TProfile> histogramProfile = this->bookProfilehistoTruth(mainNode, var1, var2, itruth);
 
             variableHistoProfile.setHisto(histogramProfile);
 
             result.addVariableHistoProfile(std::move(variableHistoProfile));
         }
 
         // add 3D histograms
         for (const auto& ivar3D : itruth->variableCombinations3D()) {
             const Variable& var1 = itruth->variableByName(std::get<0>(ivar3D));
             const Variable& var2 = itruth->variableByName(std::get<1>(ivar3D));
             const Variable& var3 = itruth->variableByName(std::get<2>(ivar3D));
 
             const std::string name = itruth->name() + "_" + var1.name() + "_vs_" + var2.name() + "_vs_" + var3.name();
             VariableHisto3D variableHisto3D(name);
             ROOT::RDF::RResultPtr<TH3D> histogram3D = this->book3DhistoTruth(mainNode, var1, var2, var3, itruth);
 
             variableHisto3D.setHisto(histogram3D);
 
             result.addVariableHisto3D(std::move(variableHisto3D));
         }
     }
 ...
 void MainFrame::writeUnfoldingHistos(TFile* outputFile,
     ...
     for (const auto& itruth : sample->truths()) {
         ...
         for (const auto& imatch : itruth->matchedVariables()) {
             ...
             std::unique_ptr<TH1D> truth = Utils::copyHistoFromVariableHistos(truthHistos.variableHistos(), truthName);
 ...
 ROOT::RDF::RResultPtr<TH1D> MainFrame::book1DhistoTruth(ROOT::RDF::RNode node,
     ...
     const std::string definition = this->definitionVariableTruth(variable, truth);
 ...
 ROOT::RDF::RResultPtr<TH2D> MainFrame::book2DhistoTruth(ROOT::RDF::RNode node,
                                                         const Variable& variable1,
                                                         const Variable& variable2,
                                                         const std::shared_ptr<Truth>& truth) const {
 
     const std::string weightName = "weight_truth_TOTAL_" + truth->name();
     const std::string definition1 = this->definitionVariableTruth(variable1, truth);
     const std::string definition2 = this->definitionVariableTruth(variable2, truth);
 ...
 ROOT::RDF::RResultPtr<TProfile> MainFrame::bookProfilehistoTruth(ROOT::RDF::RNode node,
                                                                  const Variable& variable1,
                                                                  const Variable& variable2,
                                                                  const std::shared_ptr<Truth>& truth) const {
 ...
 ROOT::RDF::RResultPtr<TH3D> MainFrame::book3DhistoTruth(ROOT::RDF::RNode node,
                                                         const Variable& variable1,
                                                         const Variable& variable2,
                                                         const Variable& variable3,
                                                         const std::shared_ptr<Truth>& truth) const {
     ...
     const std::string definition1 = this->definitionVariableTruth(variable1, truth);
     const std::string definition2 = this->definitionVariableTruth(variable2, truth);
     const std::string definition3 = this->definitionVariableTruth(variable3, truth);
 ...
 void MainFrame::addTruthBranchesToMap(const std::string& filePath,
                                       const std::shared_ptr<Sample>& sample) {
 
     if (!sample->hasTruth()) return;
 
     for (const auto& itree : sample->uniqueTruthTreeNames()) {
         m_systReplacer.addSystematicMapFromFile(filePath, itree, sample->systematics());
     }
 }
 
 std::string MainFrame::definitionVariableTruth(const Variable& variable,
                                                const std::shared_ptr<Truth>& truth) const {
