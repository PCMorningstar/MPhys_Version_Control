...
std::pair<Variable, Variable> Utils::get2DVariablesByNameTruth(const std::shared_ptr<Sample>& sample,
                                                               const std::string& variableName) {

    for (const auto& itruth : sample->truths()) {
        auto itrVar1 = std::find_if(itruth->variables().begin(), itruth->variables().end(),
            [&itruth,&variableName](const auto& element){return StringOperations::stringStartsWith(variableName, (itruth->name() + "_" + element.name() + "_vs"));});
        auto itrVar2 = std::find_if(itruth->variables().begin(), itruth->variables().end(),
            [&itruth,&variableName](const auto& element){return StringOperations::stringEndsWith(variableName, ("vs_" + element.name()));});

        if (itrVar1 == itruth->variables().end()) continue;
        if (itrVar2 == itruth->variables().end()) continue;

        return std::make_pair(*itrVar1, *itrVar2);
    }

    // this should not happen
    LOG(ERROR) << "Cannot find the corresponding 2D truth variables for: " << variableName << "\n";
    throw std::runtime_error("");
}
...
std::vector<std::string> Utils::getBranchesFromFile(const std::unique_ptr<TFile>& file,
                                                    const std::string& treeName) {
    ...
    const std::size_t branchSize = tree->GetNbranches();
