...
void SystematicReplacer::readSystematicMapFromFile(const std::string& path,
    ...
    const std::vector<std::string> branches = Utils::getBranchesFromFile(file, treeName);
...
void SystematicReplacer::addSystematicMapFromFile(const std::string& path,
                                                  const std::string& treeName,
                                                  const std::vector<std::shared_ptr<Systematic> >& systematics) {
    ...
    std::vector<std::string> newBranches;
    const std::vector<std::string> branches = Utils::getBranchesFromFile(file, treeName);
    for (const auto& ibranch : branches) {
        auto itr = std::find(m_allBranches.begin(), m_allBranches.end(), ibranch);
        if (itr != m_allBranches.end()) continue;

        newBranches.emplace_back(ibranch);
        m_allBranches.emplace_back(ibranch);
    }

    this->addMatchSystematicVariables(newBranches, systematics);
...
void SystematicReplacer::addMatchSystematicVariables(const std::vector<std::string>& variables,
                                                     const std::vector<std::shared_ptr<Systematic> >& systematics) {
    ...
    for (const auto& isyst : systematics) {
        ...
        auto itr = m_systImpactsBranches.find(systName);
        if (itr == m_systImpactsBranches.end()) {
            m_systImpactsBranches.insert({systName, affectedBranches});
        } else {
            for (const auto& ibranch : affectedBranches) {
                if (std::find(itr->second.begin(), itr->second.end(), ibranch) != itr->second.end()) continue;

                itr->second.emplace_back(ibranch);
            }
        }
    }
    ...
    for (const auto& ielement : m_systImpactsBranches) {
        for (const auto& ivariable : ielement.second) {
            ...
            if (itr == m_branchesAffectedBySyst.end()) {
                ...
                if (std::find(itr->second.begin(), itr->second.end(), ielement.first) != itr->second.end()) continue;
