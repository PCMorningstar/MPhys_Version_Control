...
class BlockReaderSampleTruth:
    ...
    def _set_cpp_class(self):
        ...
        variable_cpp_objects = self._get_cpp_variables_objects()
        variable_names = [x.name() for x in variable_cpp_objects]

        add_2d_histograms(self._histograms_2d, self.cpp_class.addVariableCombination, f"truth block {self._name}", variable_names)

        add_3d_histograms(self._histograms_3d, self.cpp_class.addVariableCombination3D, f"truth block {self._name}", variable_names)

        # we are adding TProfiles here, the difference wrt. histograms is just the C++ method for adding them
        add_2d_histograms(self._tprofiles, self.cpp_class.addVariableForProfile, f"truth block {self._name}", variable_names)
    ...
    def _get_cpp_variables_objects(self) -> list:
        """!Build the list of variable C++ objects belonging to the truth block
        """
        variable_raw_ptrs = self.cpp_class.getVariableRawPtrs()
        result = []
